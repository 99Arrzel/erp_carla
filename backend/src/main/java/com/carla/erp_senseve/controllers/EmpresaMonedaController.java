...
public class EmpresaMonedaController {
    ...
    @PostMapping("/ultimo_cambio")
    public ResponseEntity<?> ultimo_cambio(
            @RequestHeader("Authorization") String authHeader,
            @RequestBody Map<String, String> body
    ) {
        try {
            return ResponseEntity.ok().body(empresaMonedaService.ultimo_cambio(Long.parseLong(body.get("empresa_id"))));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new ResponseMessage(e.getMessage().toString()));
        }
    }
