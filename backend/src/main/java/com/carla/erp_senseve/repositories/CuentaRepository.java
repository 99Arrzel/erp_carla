...
public interface CuentaRepository extends JpaRepository<CuentaModel, Long> {
    ...
    //Ingresos es 4.? todas tipo "Detalle"
    @Query(value = "SELECT * FROM cuentas WHERE empresa_id = ?1 AND codigo LIKE '4.%' AND tipo = 'Detalle' ORDER BY inet_truchon(codigo)", nativeQuery = true)
    List<CuentaModel> findByEmpresaIdAndIngresos(
            @Param("empresa_id") Long empresa_id
    );
