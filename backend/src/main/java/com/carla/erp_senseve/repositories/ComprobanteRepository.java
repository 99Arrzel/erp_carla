...
public interface ComprobanteRepository extends JpaRepository<ComprobanteModel, Long> {
    ...
    @Query(value = "SELECT * FROM comprobantes WHERE empresa_id = ?1 AND usuario_id = ?2 ORDER BY id DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM comprobantes WHERE empresa_id = ?1 AND (estado != 'Anulado' AND fecha BETWEEN ?2 AND ?3) ORDER BY id ASC", nativeQuery = true)
